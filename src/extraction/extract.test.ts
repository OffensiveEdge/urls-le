...
describe('URL Extraction', () => {
  describe('extractUrls', () => {
    ...
    it('should include line and column information', async () => {
      ...
      expect(result.urls[0].column).toBeGreaterThan(0)
      expect(result.urls[1].line).toBe(3)
      expect(result.urls[1].column).toBeGreaterThan(0)
    })
    ...
    it('should handle mixed URL formats', async () => {
      ...
      expect(result.urls[5].value).toBe('/path/to/resource')
    })
    ...
    it('should handle URLs with query parameters and fragments', async () => {
      ...
      expect(result.urls).toHaveLength(3)
      expect(result.urls[0].value).toBe('https://example.com/search?q=test&page=1')
      expect(result.urls[1].value).toBe('https://example.com/page#section')
      expect(result.urls[2].value).toBe('https://example.com/path?param=value#fragment')
    })
  })
})
