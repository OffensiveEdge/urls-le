...
describe('extractFromHtml', () => {
    ...
    `
		...
		expect(result[1].value).toBe('https://api.example.com/users?id=123&format=json')
	})
	...
	test('extractFromHtml: large HTML file', () => {
		const links = Array.from({ length: 1000 }, (_, i) => `<a href="https://example.com/page${i}">Page ${i}</a>`)
