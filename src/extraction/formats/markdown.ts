	...
	lines.forEach((line, lineIndex) => {
		try {
			...
			while ((match = MARKDOWN_LINK_PATTERN.exec(line)) !== null) {
				const url = match[2]
				if (url && !seenUrls.has(url) && !isInInlineCode(line, match.index ?? 0)) {
			...
			while ((match = MARKDOWN_AUTOLINK_PATTERN.exec(line)) !== null) {
				const url = match[1]
				if (url && isValidUrl(url) && !seenUrls.has(url) && !isInInlineCode(line, match.index ?? 0)) {
					seenUrls.add(url)
					const components = extractUrlComponents(url)
					urls.push({
						value: url,
			...
			while ((match = URL_PATTERN.exec(line)) !== null) {
				...
				if (urlValue && isValidUrl(urlValue) && !seenUrls.has(urlValue) && !isInInlineCode(line, match.index ?? 0)) {
			...
			while ((match = MAILTO_PATTERN.exec(line)) !== null) {
				...
				if (!seenUrls.has(urlValue) && !isInInlineCode(line, match.index ?? 0)) {
					seenUrls.add(urlValue)
					urls.push({
						value: urlValue,
						protocol: detectUrlProtocol(urlValue),
