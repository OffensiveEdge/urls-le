...
import { detectUrlProtocol, extractUrlComponents, isValidUrl } from './urlValidation'
...
export interface UrlValidationService {
	...
	validateUrls(urls: readonly Url[], config: Configuration): Promise<readonly ValidationResult[]>
	dispose(): void
}
...
export function createUrlValidationService(): UrlValidationService {
	...
	return {
		async validateUrl(url: string, _config: Configuration): Promise<ValidationResult> {
			if (disposed) {
		...
		async validateUrls(urls: readonly Url[], config: Configuration): Promise<readonly ValidationResult[]> {
			if (disposed) {
				throw new Error('URL validation service has been disposed')
			}
			...
			for (let i = 0; i < urls.length; i += batchSize) {
				...
				const batchPromises = batch.map((url) => this.validateUrl(url.value, config))
 ...
 */
export function calculateValidationStats(results: readonly ValidationResult[], startTime: number): ValidationStats {
