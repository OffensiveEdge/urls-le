...
function estimateUrlCount(content: string): number {
	...
	const httpUrls = (content.match(/https?:\/\/[^\s]+/g) || []).length
	const ftpUrls = (content.match(/ftp:\/\/[^\s]+/g) || []).length
	const mailtoUrls = (content.match(/mailto:[^\s]+/g) || []).length
	const telUrls = (content.match(/tel:[^\s]+/g) || []).length
	const relativeUrls = (content.match(/\/[^\s]*/g) || []).length

	return httpUrls + ftpUrls + mailtoUrls + telUrls + relativeUrls
}
...
describe('Safety Checks', () => {
	describe('handleSafetyChecks', () => {
		...
		it('should not proceed when file size exceeds limits', () => {
			...
			const mockDocument = { getText: () => 'this is a very long content that exceeds the limit' }
	...
	describe('estimateUrlCount', () => {
      ...
      `
        ...
        Check /contact for details
        Visit /products/category/item
      `
			const count = estimateUrlCount(content)
