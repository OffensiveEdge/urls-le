...
export function formatAccessibilityStatus(accessible: number, total: number): string {
	if (total === 0) return 'No URLs'
	const percentage = formatPercentage(accessible, total)
	return `${accessible}/${total} (${percentage}) accessible`
}

export function formatDomainDistribution(domains: Array<{ domain: string; count: number }>): string {
	if (domains.length === 0) return 'No domains'

	const total = domains.reduce((sum, d) => sum + d.count, 0)
	const topDomains = domains.slice(0, 3).map((d) => `${d.domain} (${formatPercentage(d.count, total)})`)
...
export function formatProtocolDistribution(protocols: Record<string, number>): string {
	const entries = Object.entries(protocols).filter(([, count]) => count > 0)
	if (entries.length === 0) return 'No protocols'

	const total = entries.reduce((sum, [, count]) => sum + count, 0)
	const formatted = entries.map(([protocol, count]) => `${protocol.toUpperCase()} (${formatPercentage(count, total)})`)

	return formatList(formatted)
}

export function formatTimeEstimate(remaining: number): string {
	if (remaining < 60000) return `${Math.ceil(remaining / 1000)} seconds remaining`
	if (remaining < 3600000) return `${Math.ceil(remaining / 60000)} minutes remaining`
...
export function formatRemainingTime(startTime: number, total: number, processed: number): string {
...
export function formatResultSummary(successful: number, failed: number, total: number): string {
...
export function formatQualityScore(score: number, maxScore: number = 100): string {
	const percentage = formatPercentage(score, maxScore)
	const grade = score >= 90 ? 'A' : score >= 80 ? 'B' : score >= 70 ? 'C' : score >= 60 ? 'D' : 'F'
	return `${score}/${maxScore} (${percentage}, Grade: ${grade})`
}

export function formatComplianceStatus(compliant: number, total: number, standard: string): string {
