...
export function registerSortCommand(context: vscode.ExtensionContext): void {
  const command = vscode.commands.registerCommand('urls-le.postProcess.sort', async () => {
    ...
    try {
      ...
      vscode.window.showInformationMessage(`Sorted ${sorted.length} URLs (${sortOrder.label})`)
    } catch (error) {
