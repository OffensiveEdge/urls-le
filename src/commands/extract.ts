...
export function registerExtractCommand(
	...
	const command = vscode.commands.registerCommand('urls-le.extractUrls', async () => {
		...
		try {
			...
			const result = await extractUrls(document.getText(), document.languageId)
			...
			if (!result.success) {
				const errorMessage =
					result.errors && result.errors.length > 0 ? result.errors[0]?.message || 'Unknown error' : 'Unknown error'
			...
			if (config.openResultsSideBySide) {
				...
				try {
					...
					edit.replace(document.uri, new vscode.Range(0, 0, document.lineCount, 0), formattedUrls.join('\n'))
			...
			if (config.copyToClipboardEnabled) {
				try {
					...
					if (errorMessage.includes('permission') || errorMessage.includes('access')) {
						deps.notifier.showWarning('Clipboard access denied. Extracted URLs but could not copy to clipboard.')
					} else {
