...
export function getConfiguration(): Configuration {
	...
	return Object.freeze({
		copyToClipboardEnabled: config.get<boolean>(
			'copyToClipboardEnabled',
			false,
		),
		dedupeEnabled: config.get<boolean>('dedupeEnabled', false),
		notificationsLevel: config.get<'all' | 'important' | 'silent'>(
			'notificationsLevel',
			'silent',
		),
		postProcessOpenInNewFile: config.get<boolean>(
		...
		safetyFileSizeWarnBytes: config.get<number>(
			'safety.fileSizeWarnBytes',
			1000000,
		),
		safetyLargeOutputLinesThreshold: config.get<number>(
			'safety.largeOutputLinesThreshold',
			50000,
		),
		safetyManyDocumentsThreshold: config.get<number>(
		...
		analysisIncludeSecurity: config.get<boolean>(
			'analysis.includeSecurity',
			true,
		),
		analysisIncludeAccessibility: config.get<boolean>(
			'analysis.includeAccessibility',
			true,
		),
		validationEnabled: config.get<boolean>('validation.enabled', true),
		validationTimeout: config.get<number>('validation.timeout', 5000),
		validationFollowRedirects: config.get<boolean>(
